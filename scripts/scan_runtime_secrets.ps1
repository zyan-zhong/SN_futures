...
function Test-SecretLikeLine {
    ...
    if ($Text -match '(?i)(SN_ALPHA_VANTAGE_KEY|SN_NEWSAPI_KEY|SN_MANAGED_DATA_PROXY_TOKEN)\s*=\s*(?!' + $placeholder + ')([A-Za-z0-9._\-]{8,})') { return $true }
    return $false
}
...
try:
    ...
    names = ["SN_ALPHA_VANTAGE_KEY", "SN_NEWSAPI_KEY", "SN_MANAGED_DATA_PROXY_TOKEN"]
